...
int main(){
	...
	/* segundo lock sobre semaforo recursivo -> correcto */
	if (lock(desc)<0)
		printf("error en lock de mutex. NO DEBE APARECER\n");

	printf("mutex1 duerme 2 segs.: no debe ejecutar ningun proceso, ya que prueba_mutex esta dormido y mutex2 bloqueado en mutex m1\n");
	dormir(2);

	/* cierre de semaforo: debe despertar a mutex2 */
